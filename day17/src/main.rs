...
fn read_input(filename: &String) -> Vec<char> {
    let contents = fs::read_to_string(&filename).unwrap_or_else( |error| {
        panic!("read_input(): error reading {}: {:?}", &filename, &error)
    });
...
fn place(settled: &mut BTreeSet<(i64, i64)>, jet: i64, piece: i64, max_y: i64, jets: &Vec<char>, rocks: &Vec<Vec<(i64, i64)>>) -> (i64, i64, i64) {
    ...
    return (new_jet, (piece + 1) % rocks.len() as i64, cmp::max(max_y, new_cells.iter().map(|(_, y)| *y).max().unwrap()));
}
...
fn main() {
    let mut jets: Vec<char> = read_input(&"input17.txt".to_string());
    if jets[jets.len() - 1] == '\n' {
        jets.pop();
    }
    ...
    let height = solve(1_000_000_000_000, &jets, &rocks);
